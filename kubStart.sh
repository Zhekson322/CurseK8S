...
sudo swapoff -a
Отключаем файл так же после перезагрузки - (если у вас не удалось, найдите другой способ, например ручным удалением через nano)
...
sudo kubeadm init \
  --apiserver-advertise-address=10.128.0.28 \ #ip адрес мастер ноды
...
Если что то не Ready, попробуйте перезапустить
